...
5. 再运行 `codex plugin list --json`，确认 `image-control@codex-image-control` 的版本是 `0.11.2`，且 `installed` 和 `enabled` 都为 `true`。把实际版本、市场来源和验证结果告诉我。
6. 不要运行仓库中的开发脚本，不需要执行 `npm install`、重新构建或启动独立 HTTP/浏览器工作台。
7. 提醒我完全退出并重新打开 Codex，然后新建任务，在输入框键入 `@`、选择“图片生成中控”，再发送“打开图片生成中控”。
